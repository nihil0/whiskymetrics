...
class TestWhiskyBot(unittest.TestCase):
    ...
    def test_legit_post(self):
        text=self.bot.get_review_text(post_id='3rywqp')
        with open('static/sample_text.txt','r') as f:
            sample_text = f.read()

        self.assertEqual(text,sample_text)

        text = self.bot.get_review_text(url='http://www.reddit.com/r/bourbon/comments/3rywqp/review_59_1792_port_finish/')
        self.assertEqual(text,sample_text)
